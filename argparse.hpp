...
private:
    ...
    /**
        Function which prints a given error message, and optionally shows the usage.
    */
...
public:
    ...
    void parse(const StringVector& argv) {
        ...
        for (StringVector::const_iterator in = argv.begin() + ignore_first_;
            ...
            // If one of the parameters is "-h", print usage and return.
            if (el.compare("-h") == 0) {
    ...
    // Function that constructs a usage string for the program.
    String usage() {
    ...
    // Function which returns whether the parsed set of arguments is empty.
    bool empty() const {
        return index_.empty();
    }

    // Function that clears all of the captured arguments.
    void clear() {
        ignore_first_ = true;
        required_ = 0;
        index_.clear();
    ...
    // Function which returns whether the parsed set of arguments contains a certain key.
